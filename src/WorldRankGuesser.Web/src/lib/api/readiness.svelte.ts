 ...
 * calls go one at a time, `intervalMs` after each answer, for `timeoutMs`. The first call can itself hang while the
 * database resumes, so after `slowAfterMs` of silence the phase is `waking`. This is server state, not a game rule.
 */
