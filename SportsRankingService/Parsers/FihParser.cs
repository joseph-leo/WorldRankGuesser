...
public sealed class FihParser : JsonRankingParser<FihParser.Root>
{
    ...
    public sealed record Team(short Rank, [property: JsonPropertyName("team_short_code")] string TeamShortCode);

    protected override IEnumerable<RankEntry> Map(Root root) =>
        root.Ranks.Select(t => new RankEntry(t.Rank, t.TeamShortCode.Trim().IOCToISO3()));
}
