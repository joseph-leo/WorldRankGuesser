...
public sealed class SvnsParser : JsonRankingParser<SvnsParser.Root>
{
    ...
    public sealed record Entry(short Position, Team Team);
    public sealed record Team(string Abbreviation, string? Name);

    protected override IEnumerable<RankEntry> Map(Root root) =>
        root.Entries.Select(e => new RankEntry(e.Position, e.Team.Abbreviation.Trim().IOCToISO3(), e.Team.Name));
}
