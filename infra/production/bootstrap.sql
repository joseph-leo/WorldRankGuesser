-- The production database's users: run by the owner, connected as the server's Entra admin to the WorldRankGuesser
...
-- idempotent. Staging: infra/staging/bootstrap.sql. A managed identity's user name is the identity's name.
...
IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = N'id-wrg-production-deploy')
    CREATE USER [id-wrg-production-deploy] FROM EXTERNAL PROVIDER;
ALTER ROLE db_owner ADD MEMBER [id-wrg-production-deploy];

-- The scraper reads and writes its own schema, dbo; its migrations are the deploy identity's job.
IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = N'id-wrg-production-scraper')
    CREATE USER [id-wrg-production-scraper] FROM EXTERNAL PROVIDER;
GRANT SELECT, INSERT, UPDATE, DELETE ON SCHEMA::dbo TO [id-wrg-production-scraper];

-- The game reads one view (ownership chaining covers the tables beneath it) and owns its data in schema game.
IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = N'id-wrg-production-game')
    CREATE USER [id-wrg-production-game] FROM EXTERNAL PROVIDER;
GRANT SELECT, INSERT, UPDATE, DELETE ON SCHEMA::game TO [id-wrg-production-game];
IF OBJECT_ID(N'dbo.CurrentCountryRankings', N'V') IS NOT NULL
    GRANT SELECT ON OBJECT::dbo.CurrentCountryRankings TO [id-wrg-production-game];
ELSE
